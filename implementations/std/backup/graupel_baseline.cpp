...
void graupel(size_t &nvec, size_t &ke, size_t &ivstart, size_t &ivend,
 ...
 array_1d_t<size_t> kmin(nvec * np, ke + 1); // first level with condensate
  ...
  std::exclusive_scan(std::execution::par, flags.begin(), flags.end(), prefixsum.begin(), 0);

  std::for_each(std::execution::par, indices_.begin(), indices_.end(),
  [=, flags = flags.data(), prefixsum = prefixsum.data(), ind_i = ind_i.data(), ind_j = ind_j.data()](size_t j) {
  ...
  std::for_each(std::execution::par, indices.begin(), indices.end(),
    [=, p = p.data(), t = t.data(), q = q.data(), rho = rho.data(),
    ind_i = ind_i.data(), ind_j = ind_j.data(), qp_ind = qp_ind_copy, qx_ind = qx_ind_copy](size_t j) {
  ...
  std::for_each(std::execution::par, indices_.begin(), indices_.end(),
    [=, kmin = kmin.data(), q = q.data(), dz = dz.data(),
