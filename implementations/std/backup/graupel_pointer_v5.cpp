...
void graupel(size_t &nvec, size_t &ke, size_t &ivstart, size_t &ivend,
  ...
  size_t jmx_ = std::transform_reduce(std::execution::par_unseq, indices_.begin(), indices_.end(), size_t(0), std::plus<size_t>(),
    [=](size_t j) {
      size_t count = 0;
      ...
      for (size_t i = ke - 1; i < ke; --i) {  
        ...
        flags_ptr[i * ivend + j] = (cond1 || cond2) * 1;
        count += (cond1 || cond2);
      ...
      kmin_ptr[j * np + qp_ind[0]] = var0;
      kmin_ptr[j * np + qp_ind[1]] = var1;
      kmin_ptr[j * np + qp_ind[2]] = var2;
      kmin_ptr[j * np + qp_ind[3]] = var3;
      return count;
    }
  );
  ...
  [=](size_t j) {
      for (size_t i = ke - 1; i < ke; --i) {        
        if (flags_ptr[i * ivend + j]) {
          ind_i_ptr[prefixsum_ptr[i * ivend + j]] = i;
          ind_j_ptr[prefixsum_ptr[i * ivend + j]] = j;
        }
      }
  });
  ...
  std::for_each(std::execution::par_unseq, indices.begin(), indices.end(),
    [=](size_t j) {
        ...
        bool is_sig_present = std::max({x_array_ptr[lqs][k * ivend + iv], x_array_ptr[lqi][k * ivend + iv], x_array_ptr[lqg][k * ivend + iv]}) > qmin;

        dvsw = x_array_ptr[lqv][k * ivend + iv] -
               qsat_rho(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv]);
        qvsi = qsat_ice_rho(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv]);
        dvsi = x_array_ptr[lqv][k * ivend + iv] - qvsi;
        n_snow = snow_number(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv],
                              x_array_ptr[lqs][k * ivend + iv]);
        l_snow = snow_lambda(rho_ptr[k * ivend + iv], x_array_ptr[lqs][k * ivend + iv], n_snow);
    
        sx2x[lqc][lqr] = cloud_to_rain(t_ptr[k * ivend + iv], x_array_ptr[lqc][k * ivend + iv],
                                        x_array_ptr[lqr][k * ivend + iv], qnc);
        sx2x[lqr][lqv] = rain_to_vapor(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv],
                                      x_array_ptr[lqc][k * ivend + iv],
                                      x_array_ptr[lqr][k * ivend + iv], dvsw, dt);
        sx2x[lqc][lqi] = cloud_x_ice(t_ptr[k * ivend + iv], x_array_ptr[lqc][k * ivend + iv],
                                      x_array_ptr[lqi][k * ivend + iv], dt);
        sx2x[lqi][lqc] = -std::fmin(sx2x[lqc][lqi], ZERO);
        sx2x[lqc][lqi] = std::fmax(sx2x[lqc][lqi], ZERO);
        sx2x[lqc][lqs] = cloud_to_snow(t_ptr[k * ivend + iv], x_array_ptr[lqc][k * ivend + iv],
                                        x_array_ptr[lqs][k * ivend + iv], n_snow, l_snow);
        sx2x[lqc][lqg] =
            cloud_to_graupel(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv],
                            x_array_ptr[lqc][k * ivend + iv], x_array_ptr[lqg][k * ivend + iv]);
    
        if (t_ptr[k * ivend + iv] < tmelt) {
          n_ice = ice_number(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv]);
          m_ice = ice_mass(x_array_ptr[lqi][k * ivend + iv], n_ice);
          x_ice = ice_sticking(t_ptr[k * ivend + iv]);
    
          if (is_sig_present) {
            eta = deposition_factor(
                t_ptr[k * ivend + iv],
                qvsi); // neglect cloud depth cor. from gcsp_graupel
            sx2x[lqv][lqi] = vapor_x_ice(x_array_ptr[lqi][k * ivend + iv], m_ice, eta, dvsi,
                                         rho_ptr[k * ivend + iv], dt);
            sx2x[lqi][lqv] = -std::fmin(sx2x[lqv][lqi], ZERO);
            sx2x[lqv][lqi] = std::fmax(sx2x[lqv][lqi], ZERO);
            ice_dep = std::fmin(sx2x[lqv][lqi], dvsi / dt);
    
            sx2x[lqi][lqs] = deposition_auto_conversion(x_array_ptr[lqi][k * ivend + iv],
                                                        m_ice, ice_dep);
            sx2x[lqi][lqs] = sx2x[lqi][lqs] + ice_to_snow(x_array_ptr[lqi][k * ivend + iv],
                                                          n_snow, l_snow, x_ice);
            sx2x[lqi][lqg] = ice_to_graupel(
                rho_ptr[k * ivend + iv], x_array_ptr[lqr][k * ivend + iv],
                x_array_ptr[lqg][k * ivend + iv], x_array_ptr[lqi][k * ivend + iv], x_ice);
            sx2x[lqs][lqg] =
                snow_to_graupel(t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv],
                                x_array_ptr[lqc][k * ivend + iv], x_array_ptr[lqs][k * ivend + iv]);
            sx2x[lqr][lqg] = rain_to_graupel(
                t_ptr[k * ivend + iv], rho_ptr[k * ivend + iv], x_array_ptr[lqc][k * ivend + iv],
                x_array_ptr[lqr][k * ivend + iv], x_array_ptr[lqi][k * ivend + iv],
                x_array_ptr[lqs][k * ivend + iv], m_ice, dvsw, dt);
          }
          sx2x[lqv][lqi] =
              sx2x[lqv][lqi] +
              ice_deposition_nucleation(t_ptr[k * ivend + iv], x_array_ptr[lqc][k * ivend + iv],
                                        x_array_ptr[lqi][k * ivend + iv], n_ice, dvsi, dt);
        } else {
        ...
        if (is_sig_present) {
          dvsw0 = x_array_ptr[lqv][k * ivend + iv] - qsat_rho(tmelt, rho_ptr[k * ivend + iv]);
          sx2x[lqv][lqs] =
              vapor_x_snow(t_ptr[k * ivend + iv], p_ptr[k * ivend + iv],
                           rho_ptr[k * ivend + iv], x_array_ptr[lqs][k * ivend + iv], n_snow,
          ...
          sx2x[lqv][lqg] = vapor_x_graupel(
              t_ptr[k * ivend + iv], p_ptr[k * ivend + iv], rho_ptr[k * ivend + iv],
              x_array_ptr[lqg][k * ivend + iv], dvsw, dvsi, dvsw0, dt);
          sx2x[lqg][lqv] = -std::fmin(sx2x[lqv][lqg], ZERO);
          sx2x[lqv][lqg] = std::fmax(sx2x[lqv][lqg], ZERO);
          sx2x[lqs][lqr] =
              snow_to_rain(t_ptr[k * ivend + iv], p_ptr[k * ivend + iv],
                           rho_ptr[k * ivend + iv], dvsw0, x_array_ptr[lqs][k * ivend + iv]);
          sx2x[lqg][lqr] =
              graupel_to_rain(t_ptr[k * ivend + iv], p_ptr[k * ivend + iv],
                              rho_ptr[k * ivend + iv], dvsw0, x_array_ptr[lqg][k * ivend + iv]);
        }
        ...
        for (size_t ix = 0; ix < nx; ix++) {
          ...
          if ((is_sig_present) or (qx_ind[ix] == lqc) or (qx_ind[ix] == lqv) or
            ...
            stot = x_array_ptr[qx_ind[ix]][k * ivend + iv] / dt;
    
            if ((sink[qx_ind[ix]] > stot) &&
                (x_array_ptr[qx_ind[ix]][k * ivend + iv] > qmin)) {
        ...
        for (size_t ix = 0; ix < nx; ix++) {
          ...
          x_array_ptr[qx_ind[ix]][k * ivend + iv] = std::fmax(
              ZERO, x_array_ptr[qx_ind[ix]][k * ivend + iv] + dqdt[qx_ind[ix]] * dt);
        }
    
        qice = x_array_ptr[lqs][k * ivend + iv] + x_array_ptr[lqi][k * ivend + iv] + x_array_ptr[lqg][k * ivend + iv];
        qliq = x_array_ptr[lqc][k * ivend + iv] + x_array_ptr[lqr][k * ivend + iv];
        qtot = x_array_ptr[lqv][k * ivend + iv] + qice + qliq;
        cv = cvd + (cvv - cvd) * qtot + (clw - cvv) * qliq +
             (ci - cvv) * qice; // qtot? or qv?
        t_ptr[k * ivend + iv] =
            t_ptr[k * ivend + iv] +
            dt *
                ((dqdt[lqc] + dqdt[lqr]) * (lvc - (clw - cvv) * t_ptr[k * ivend + iv]) +
                 (dqdt[lqi] + dqdt[lqs] + dqdt[lqg]) *
                     (lsc - (ci - cvv) * t_ptr[k * ivend + iv])) /
                cv;
  });
  ...
  std::for_each(std::execution::par_unseq, indices_.begin(), indices_.end(),
      ...
      for (size_t k = kstart; k < k_end; k++) {
        ...
        e_int =
            internal_energy(t_ptr[k * ivend + iv], x_array_ptr[lqv][k * ivend + iv], qliq,
                            qice, rho_ptr[k * ivend + iv], dz_ptr[k * ivend + iv]) +
            eflx;
        zeta = dt / (2.0 * dz_ptr[k * ivend + iv]);
        xrho = std::sqrt(rho_00 / rho_ptr[k * ivend + iv]);
        
        #pragma unroll np
        for (size_t ix = 0; ix < np; ix++) {
          if (k < kmin_ptr[iv * np + qp_ind[ix]])
            continue;
          vc = vel_scale_factor(qp_ind[ix], xrho, rho_ptr[k * ivend + iv],
                                t_ptr[k * ivend + iv],
                                x_array_ptr[qp_ind[ix]][k * ivend + iv]);
          precip(params[qp_ind[ix]], update, zeta, vc, p_array_ptr[qp_ind[ix]][iv],
                  vt[ix], x_array_ptr[qp_ind[ix]][k * ivend + iv],
                  x_array_ptr[qp_ind[ix]][kp1 * ivend + iv], rho_ptr[k * ivend + iv]);
                  x_array_ptr[qp_ind[ix]][k * ivend + iv] = update[0];
                  p_array_ptr[qp_ind[ix]][iv] = update[1];
          vt[ix] = update[2];
        }

        pflx_ptr[k * ivend + iv] = p_array_ptr[lqs][iv] + p_array_ptr[lqi][iv] + p_array_ptr[lqg][iv];
        eflx =
            dt * (p_array_ptr[lqr][iv] * (clw * t_ptr[k * ivend + iv] -
                                  cvd * t_ptr[kp1 * ivend + iv] - lvc) +
                  pflx_ptr[k * ivend + iv] * (ci * t_ptr[k * ivend + iv] -
                                          cvd * t_ptr[kp1 * ivend + iv] - lsc));
        pflx_ptr[k * ivend + iv] = pflx_ptr[k * ivend + iv] + p_array_ptr[lqr][iv];
        qliq = x_array_ptr[lqc][k * ivend + iv] + x_array_ptr[lqr][k * ivend + iv];
        qice = x_array_ptr[lqs][k * ivend + iv] + x_array_ptr[lqi][k * ivend + iv] +
        x_array_ptr[lqg][k * ivend + iv];
        e_int = e_int - eflx;
        t_ptr[k * ivend + iv] =
            T_from_internal_energy(e_int, x_array_ptr[lqv][k * ivend + iv], qliq, qice,
                                    rho_ptr[k * ivend + iv], dz_ptr[k * ivend + iv]);
        flag = (k == ke - 1);
